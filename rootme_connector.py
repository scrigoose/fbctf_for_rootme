...
def retrieve_stats(user):
    ...
    r = requests.get(config.params['url_stats'].format(user), proxies=config.params['proxies'])
    ret = dict()
...
def retrieve_points(users):
    ...
    scores = []
    for user in users:
        r = requests.get(config.params['url_rootme'].format(user), proxies=config.params['proxies'])
        if r.status_code == 200:
